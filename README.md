...
3. **Pick a model by provider prefix.** The provider is the first segment of the model name: `gemini-cli/<model>`, `antigravity/<model>` or `aicode/<experience>` — e.g. `gemini-cli/gemini-2.5-pro`, `gemini-cli/gemini-2.5-flash`, `antigravity/claude-sonnet-4-6`, `antigravity/gemini-3-pro-high`, `aicode/gemini-3.7-flash-high` (the last needs an `[aicode]` config table; it reuses the `gemini-cli` login rather than adding one of its own). The part after the prefix is sent upstream as-is, so any model your account can serve works (not just catalogued ones). `GET /v1beta/models` lists the known models (provider-prefixed) for the providers you have credentials for. Streaming (`:streamGenerateContent`) and `:countTokens` are supported.
